ALTER TABLE public.ecer_preparations ADD COLUMN IF NOT EXISTS photo_paths text[] NOT NULL DEFAULT '{}'::text[];

UPDATE public.ecer_preparations e
SET photo_paths = s.photo_paths
FROM public.prep_submissions s
WHERE e.prep_submission_id = s.id
  AND coalesce(array_length(s.photo_paths,1),0) > 0
  AND coalesce(array_length(e.photo_paths,1),0) < coalesce(array_length(s.photo_paths,1),0);

UPDATE public.ecer_preparations
SET photo_paths = ARRAY[photo_path]::text[]
WHERE coalesce(array_length(photo_paths,1),0) = 0
  AND photo_path IS NOT NULL AND length(trim(photo_path)) > 0;
