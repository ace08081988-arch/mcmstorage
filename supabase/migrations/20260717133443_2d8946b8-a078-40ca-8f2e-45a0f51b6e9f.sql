DELETE FROM public.prep_task_request_titles ptrt
USING public.prep_tasks t, public.request_titles rt
WHERE ptrt.task_id = t.id
  AND ptrt.title_id = rt.id
  AND t.title ~* '^Request:\s*'
  AND t.status = 'active'
  AND t.expires_at > now()
  AND rt.user_id = t.owner_user_id
...
BEGIN
  ...
  JOIN public.prep_task_request_titles ptrt
    ON ptrt.title_id = t.id
   AND ptrt.task_id = v_task.id
...
REVOKE ALL ON FUNCTION public.request_list_titles_via_task(text, text) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.request_list_titles_via_task(text, text) TO anon, authenticated, service_role;