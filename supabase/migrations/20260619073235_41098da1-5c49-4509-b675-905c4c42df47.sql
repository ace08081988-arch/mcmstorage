...
CREATE OR REPLACE FUNCTION public.prep_submit(_token text, _pin text, _task_item_id uuid, _photo_path text, _location_url text, _gps_lat double precision, _gps_lng double precision, _note text, _qty_reported numeric)
 ...
 SET search_path TO 'public', 'extensions'
AS $function$
DECLARE v_task public.prep_tasks%ROWTYPE; v_item public.prep_task_items%ROWTYPE; v_sub_id uuid; v_stock numeric;
BEGIN
  ...
  IF _qty_reported IS NOT NULL AND _qty_reported > 0 THEN
    UPDATE public.prep_task_items
      SET qty_prepared = COALESCE(qty_prepared,0) + _qty_reported
      WHERE id = v_item.id;

    -- Kurangi stok gudang induk sesuai jumlah yang disiapkan pegawai
    IF v_item.warehouse_item_id IS NOT NULL THEN
      ...
      END IF;
      IF v_stock < _qty_reported THEN
        RETURN jsonb_build_object('ok', false, 'error', 'insufficient_stock',
          'available', v_stock, 'requested', _qty_reported);
      END IF;
      UPDATE public.warehouse_items
        SET stock_base = stock_base - _qty_reported, updated_at = now()
        WHERE id = v_item.warehouse_item_id AND user_id = v_task.owner_user_id;
    END IF;
  END IF;
