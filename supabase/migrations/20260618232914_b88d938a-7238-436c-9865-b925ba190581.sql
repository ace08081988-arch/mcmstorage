-- Restore private cron secret for /lovable/email/queue/process
...
WHERE id = 1 AND (cron_secret IS NULL OR cron_secret = '');
...
DECLARE
  ...
  v_existing_id UUID;
BEGIN
  SELECT cron_secret INTO v_secret FROM public.email_send_state WHERE id = 1;
  IF v_secret IS NULL OR v_secret = '' THEN
    RAISE EXCEPTION 'email_send_state.cron_secret is not configured';
  END IF;

  SELECT id INTO v_existing_id FROM vault.secrets WHERE name = 'email_queue_cron_secret';
