...
function PortalErrorLogPage() {
  ...
  if (isCheckingAdmin) {
    return <div className="p-4 text-sm text-muted-foreground">Memeriksa akses…</div>;
  }
  if (!isAdmin) {
    return (
      <div className="mx-auto max-w-lg p-6 text-center">
        ...
        <Link to="/" className="mt-3 inline-block text-sm underline">Kembali</Link>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-5xl space-y-4 p-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">Log Error Portal Publik</h1>
        </div>
        <Button
          size="sm"
          variant="outline"
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
        <Card>
          <CardHeader className="pb-1"><CardTitle className="text-xs uppercase text-muted-foreground">Event 24 jam</CardTitle></CardHeader>
          <CardContent className="text-2xl font-bold">{totals?.events24h ?? 0}</CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-1"><CardTitle className="text-xs uppercase text-muted-foreground">Alert terbuka</CardTitle></CardHeader>
          <CardContent className="text-2xl font-bold text-destructive">{totals?.openAlerts ?? 0}</CardContent>
        </Card>
        <Card className="col-span-2 md:col-span-1">
          <CardHeader className="pb-1"><CardTitle className="text-xs uppercase text-muted-foreground">Top kind (24 jam)</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-xs">
            {(totals?.byKind ?? []).slice(0, 5).map((k) => (
              <button
                key={k.kind}
                type="button"
                className="mr-1 rounded border px-2 py-0.5 hover:bg-muted"
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {alerts.length === 0 && (
            <div className="text-xs text-muted-foreground">Belum ada alert.</div>
          )}
          {alerts.map((a) => (
            <div key={a.id} className="flex flex-wrap items-center justify-between gap-2 rounded border px-3 py-2 text-xs">
              <div className="flex items-center gap-2">
      ...
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">
            Event terbaru {kindFilter && <span className="ml-2 text-xs text-muted-foreground">filter: {kindFilter} <button className="ml-1 underline" onClick={() => setKindFilter("")}>hapus</button></span>}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {events.length === 0 && (
                  <tr><td colSpan={7} className="py-4 text-center text-muted-foreground">Tidak ada event.</td></tr>
                )}
                {events.map((e) => (
