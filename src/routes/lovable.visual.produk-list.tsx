...
function HeroCard({ name, product, target, ref }: typeof HERO_FIXTURES[number]) {
  return (
    <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
      <div className="relative bg-gradient-to-br from-primary/95 via-primary to-primary/80 px-ms-4 pb-4 pt-4 text-primary-foreground">
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-ms-3">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-ms-1.5 text-ms-2xs leading-none text-primary-foreground/85">
              <span className="inline-flex h-6 min-w-0 max-w-full items-center gap-ms-1 rounded-full bg-white/15 px-ms-2 leading-none">
  ...
  return (
    <div className="rounded-lg border bg-card p-ms-3 space-ms-2">
      {STATUS_VARIANTS.map((s) => (
        <div key={s.variant} className="flex items-start justify-between gap-ms-2">
          <div className="min-w-0">
...
function DetailRows() {
  return (
    <div className="rounded-xl border bg-card divide-y px-ms-4">
...
function VisualHarness() {
  ...
  return (
    <div className="min-h-screen bg-background p-ms-3 text-foreground" data-press-scope="on">
      <div className="mx-auto max-w-3xl space-ms-4">
        {(part === "all" || part === "hero") && (
          <section data-visual-part="hero" className="space-ms-3">
            {HERO_FIXTURES.map((h) => (
