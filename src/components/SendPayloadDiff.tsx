...
export function SendPayloadDiff({
  ...
  return (
    <div className="mt-2 rounded-md border border-rose-500/30 bg-background/60 text-ms-2xs">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between gap-ms-2 px-ms-2 py-1.5 text-left font-medium text-rose-900 dark:text-rose-200"
        aria-expanded={open}
      >
        <span className="inline-flex items-center gap-ms-1">
          {open ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
          Detail perbedaan payload
          <span className="ml-1 rounded-full bg-rose-500/15 px-1.5 py-0.5 text-ms-2xs font-semibold">
      ...
      {open ? (
        <div className="border-t border-rose-500/20 px-ms-2 py-1.5">
          {!previous ? (
            ...
            <dl className="grid grid-cols-1 gap-ms-1.5">
              {rows.map((row) => (
                <div key={row.label} className="rounded border bg-card/60 p-ms-1.5">
                  ...
                  {row.changed ? (
                    <dd className="mt-1 grid grid-cols-1 gap-ms-1 sm:grid-cols-2">
...
function ValueBlock({ tone, label, value }: { tone: "prev" | "curr"; label: string; value: string }) {
  return (
    ...
    >
      ...
      <div className="whitespace-pre-wrap break-words text-ms-2xs text-foreground/90">
...
function buildRows(prev: SendPayloadSummary | null, curr: SendPayloadSummary | null): Row[] {
  ...
  const fields: { key: keyof SendPayloadSummary; label: string; fmt: (v: unknown) => string }[] = [
    { key: "channel", label: "Channel", fmt: (v) => (v === "wa" ? "Ace" : v === "chat" ? "Chat aplikasi" : "—") },
