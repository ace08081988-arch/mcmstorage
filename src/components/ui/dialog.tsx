...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-overlay bg-background/70 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className,
    )}
    {...props}
  />
    ...
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        // tetap terlihat. Padding lebih kompak di HP, generous di ≥sm.
          ...
          "group/dialog fixed left-[50%] top-[50%] z-overlay flex w-[calc(100%-1rem)] max-w-lg translate-x-[-50%] translate-y-[-50%] flex-col gap-4 border bg-background p-4 depth-3d-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 rounded-lg overflow-y-auto overscroll-contain scroll-pt-24 scroll-pb-6 focus:outline-none [max-height:calc(var(--app-vh-visible,var(--app-vh,100dvh))-max(var(--app-safe-top,var(--app-safe-top,env(safe-area-inset-top,0px))),1rem)-max(var(--app-safe-bottom,var(--app-safe-bottom,env(safe-area-inset-bottom,0px))),1rem))] sm:w-full sm:p-6",
          className,
        )}
        {...props}
      >
...
const DialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
      "flex flex-col-reverse gap-ms-2 sm:flex-row sm:justify-end",
      // Tombol aksi tetap terjangkau walau isi dialog panjang / layar pendek.
      "sticky bottom-0 z-10 mt-auto border-t border-transparent bg-background pt-2",
      ...
      "group-data-[scrolled=true]/dialog:border-border",
      className,
    )}
    {...props}
  />
