    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="chat-field-scope max-w-lg">
        ...
        <div className="space-ms-3">
          <div>
            <div className="mb-1 text-ms-xs font-semibold text-muted-foreground">Asli</div>
            <div className="max-h-32 overflow-y-auto rounded-md border bg-muted/30 p-ms-2 text-ms-sm whitespace-pre-wrap">{source}</div>
          </div>
          <div className="flex items-center justify-between gap-ms-2">
            <div className="text-ms-xs font-semibold text-muted-foreground">Terjemahan ({target === "id" ? "Indonesia" : "Inggris"})</div>
          ...
          <div className="min-h-20 rounded-md border bg-background p-ms-2 text-ms-sm whitespace-pre-wrap">
            {loading ? (
              <span className="inline-flex items-center gap-ms-2 text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" /> Menerjemahkan…</span>
            ) : err ? (
