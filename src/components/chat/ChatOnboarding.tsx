...
export function ChatOnboarding({ onStartDm, onNewGroup, onShowQr }: Props) {
    ...
    return (
      <div className="rounded-2xl border bg-card p-8 text-center">
        ...
        <div className="mt-4 flex flex-wrap justify-center gap-ms-2">
          <Button size="sm" asChild className="gap-ms-1.5">
            <Link to="/kontak">
              <UserPlus className="h-4 w-4" /> Tambah kontak
            </Link>
          </Button>
          {onStartDm ? (
            <Button size="sm" variant="outline" className="gap-ms-1.5" onClick={onStartDm}>
  ...
  const steps = [
    {
      ...
      cta: (
        <Button asChild size="sm" variant="secondary" className="gap-ms-1.5">
          <Link to="/kontak">
    ...
    },
    {
      ...
      cta: onStartDm ? (
        <Button size="sm" variant="secondary" className="gap-ms-1.5" onClick={onStartDm}>
    ...
    },
    {
      ...
      cta: onNewGroup ? (
        <Button size="sm" variant="secondary" className="gap-ms-1.5" onClick={onNewGroup}>
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border bg-gradient-to-br from-primary/10 via-card to-card p-ms-4 sm:p-ms-5">
      <button
        type="button"
        onClick={dismiss}
        aria-label="Tutup panduan"
        className="absolute right-2 top-2 rounded-full p-ms-1.5 text-muted-foreground transition hover:bg-accent hover:text-foreground"
      >
        <X className="h-4 w-4" />
      </button>
      ...
      <ol className="mt-4 space-ms-3">
        {steps.map((s, i) => (
          <li
            key={s.title}
            className="flex items-start gap-ms-3 rounded-xl border bg-background/60 p-ms-3"
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-ms-2">
                <span className="text-ms-2xs font-semibold text-muted-foreground">
                  LANGKAH {i + 1}
                </span>
              </div>
              <p className="text-ms-sm font-medium leading-snug">{s.title}</p>
              <p className="mt-0.5 text-ms-xs text-muted-foreground">{s.desc}</p>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-ms-2 border-t pt-3">
        {onShowQr ? (
          <Button variant="ghost" size="sm" className="gap-ms-1.5" onClick={onShowQr}>
        ...
        <button
          type="button"
          onClick={dismiss}
          className="text-ms-xs font-medium text-muted-foreground hover:text-foreground"
        >
          Nanti saja
        </button>
      </div>
